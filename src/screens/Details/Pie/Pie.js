...
export default function DetailsScreen() {
  ...
  const getTransactions = async function () {
    const { data: response } = await axios.get('/api/v1/user/transactions_to_chart', {
      params: {
        month: date.month() + 1,
        year: date.year(),
      },
  ...
  } = useQuery('transaction_chart', getTransactions);
  const {
    data: income_data,
    status: income_status,
    refetch: refetch_income,
  } = useQuery('income_pie_chart', getIncomeChart);
